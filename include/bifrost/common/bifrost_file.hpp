...
namespace bifrost {
    ...
    public:
        ...
        bool recv_file(asio::ip::tcp::socket &socket) {
            try{
                buffer_type buf;
...
//                            std::cout << "got " << len << " but need " << left_file_size << std::endl;
        ...
        bool send_file(asio::ip::tcp::socket & socket,const std::list<std::string>& files) {
            if(files.empty()){
                return false;
            }
            ...
            try
            {
                ...
                for(auto& file_info : file_header.files){
                    ...
                    while(true){
                        ...
                        asio::write(socket, asio::buffer(buf.data(),buf.size()),asio::transfer_all(), error);
                        if (error){
